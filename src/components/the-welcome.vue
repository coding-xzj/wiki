...
const getStatistic = async () => {
  const res = await axios.get("/ebook-snapshot/get-statistic");
  const data = res.data;
  if (data.success) {
    ...
    if (statisticResp.length > 1) {
      ...
      statistic.value.todayViewIncrease = parseInt(
        String(statisticResp[1].viewIncrease / nowRate)
      );
      ...
      // todayViewIncreaseRate：今日预计增长率
      statistic.value.todayViewIncreaseRate =
        statistic.value.todayViewIncrease * 100;
      statistic.value.todayViewIncreaseRateAbs = Math.abs(
        statistic.value.todayViewIncreaseRate
      );
    }
...
const get30DayStatistic = async () => {
  const res = await axios.get("/ebook-snapshot/get-30-statistic");
  const data = res.data;
  if (data.success) {
